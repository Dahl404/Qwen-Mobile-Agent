...
int selfctx_detect(const char *path, selfctx_hdr_t *hdr) {
    ...
    if (fd < 0) return 0;
    struct stat st;
    if (fstat(fd, &st) != 0 || st.st_size < (off_t)SELFCTX_HDR_LEN) {
        close(fd);
        return 0;
    }
    selfctx_hdr_t h;
    ...
    /* sanity: offsets must look sane and in order */
    if (h.base_size == 0 || h.base_size > (uint64_t)st.st_size) return 0;
    if (h.model_size > 0) {
        if (h.model_off < h.base_size) return 0;
        if (h.model_off + h.model_size > h.int_off) return 0;
    } else if (h.int_off < h.base_size) {
        return 0;
    }
    if (h.int_size > 0 && h.cfg_off > 0 && h.int_off + h.int_size > h.cfg_off) return 0;
...
char *selfctx_snapshot_gen(const char *src, const char *dir, const char *out_dir,
    ...
    uint64_t src_model_off = 0, src_model_size = 0;
    int mfd = -1;   /* fd of the binary the model offsets came from (may
                       differ from src: the exit pipeline compiles a fresh
                       base but copies the net forward from the running
                       binary — model_src). */
    {
        selfctx_hdr_t old;
        const char *msrc = model_src ? model_src : src;
        if (selfctx_detect(src, &old)) hdr.base_size = old.base_size;
        if (selfctx_detect(msrc, &old)) {
            src_model_off = old.model_off;
            src_model_size = old.model_size;
            if (strcmp(msrc, src) != 0) mfd = open(msrc, O_RDONLY);
        }
    }
    ...
    /* layout: [base][model][internal][pad][cfg][kv][state][salience][header] */
    hdr.model_off = align_up(hdr.base_size, PAGE);
    hdr.model_size = src_model_size;
    hdr.int_off = align_up(hdr.model_off + hdr.model_size, PAGE);
    ...
    /* pad base → model_off, copy the embedded model section forward. The
       model bytes come from the binary its offsets were read from (mfd),
       NOT from src_fd — src may be a freshly compiled base with no model. */
    if (ftruncate(dst, (off_t)hdr.model_off) != 0) goto fail;
    if (hdr.model_size > 0) {
        if (copy_plain_at(mfd >= 0 ? mfd : src_fd, (off_t)src_model_off, dst,
                          (off_t)hdr.model_off, (size_t)hdr.model_size) != 0) goto fail;
    } else if (model_src) {
        /* the caller explicitly wanted the model carried forward (the exit
           pipeline) but the source has none — fail loudly instead of
           silently producing a model-less generation, so the caller can
           fall back to a self-snapshot */
        goto fail;
    }
    if (mfd >= 0) close(mfd);
    mfd = -1;
    /* pad model → int_off, then embed the internal tree */
...
/* Breed a new base executable with the model embedded (see selfctx.h).
   `base` must be a freshly built binary: [ELF][internal blob][bare header].
   The aligned GGUF is inserted between the ELF and the internal blob, and
   the tail header is rewritten with the model section. Output goes to a
   temp file, renamed over `outfile` (the running binary's inode is never
   touched — the caller then exec()s the new file). */
int selfctx_embed_model(const char *base, const char *model_file,
                        const char *outfile) {
    selfctx_hdr_t h;
    if (!selfctx_detect(base, &h)) return -1;
    int base_fd = open(base, O_RDONLY);
    if (base_fd < 0) return -1;
    int mfd = open(model_file, O_RDONLY);
    if (mfd < 0) { close(base_fd); return -1; }
    struct stat mst;
    if (fstat(mfd, &mst) != 0 || mst.st_size <= 0) { close(base_fd); close(mfd); return -1; }
    uint64_t model_size = (uint64_t)mst.st_size;
    /* preserve the base's internal blob AND its session sections (a
       model-less binary with a live session must keep it when the model
       is bred in — zeroing it used to wipe the conversation) */
    uint64_t old_int_off = h.int_off, old_int_size = h.int_size;
    uint64_t old_cfg_off = h.cfg_off, old_cfg_size = h.cfg_size;
    uint64_t old_kv_off = h.kv_off, old_kv_size = h.kv_size;
    uint64_t old_st_off = h.st_off, old_st_size = h.st_size;
    uint64_t old_sal_off = h.sal_off, old_sal_size = h.sal_size;
    uint64_t old_sysfp = h.sysfp, old_n_pos = h.n_pos, old_flags = h.flags;

    h.model_off = align_up(h.base_size, PAGE);
    h.model_size = model_size;
    h.int_off = align_up(h.model_off + model_size, PAGE);
    /* re-lay the preserved sections after the internal blob */
    h.cfg_off = align_up(h.int_off + old_int_size, PAGE);
    h.cfg_size = old_cfg_size;
    h.kv_off = h.cfg_off + old_cfg_size;
    h.kv_size = old_kv_size;
    h.st_off = h.kv_off + old_kv_size;
    h.st_size = old_st_size;
    h.sal_off = h.st_off + old_st_size;
    h.sal_size = old_sal_size;
    h.sysfp = old_sysfp; h.n_pos = old_n_pos; h.flags = old_flags;
    uint64_t total = h.sal_off + h.sal_size + sizeof(selfctx_hdr_t);

    char tmp[1200];
    snprintf(tmp, sizeof(tmp), "%s.breed.tmp", outfile);
    int dst = open(tmp, O_RDWR | O_CREAT | O_TRUNC, 0755);
    if (dst < 0) { close(base_fd); close(mfd); return -1; }
    if (copy_plain_at(base_fd, 0, dst, 0, (size_t)h.base_size) != 0) goto fail;
    if (ftruncate(dst, (off_t)h.model_off) != 0) goto fail;
    if (copy_plain_at(mfd, 0, dst, (off_t)h.model_off, (size_t)model_size) != 0) goto fail;
    if (ftruncate(dst, (off_t)h.int_off) != 0) goto fail;
    if (old_int_size > 0) {
        if (copy_plain_at(base_fd, (off_t)old_int_off, dst,
                          (off_t)h.int_off, (size_t)old_int_size) != 0) goto fail;
    }
    /* copy the preserved session sections forward */
    if (old_cfg_size > 0 &&
        copy_plain_at(base_fd, (off_t)old_cfg_off, dst,
                      (off_t)h.cfg_off, (size_t)old_cfg_size) != 0) goto fail;
    if (old_kv_size > 0 &&
        copy_plain_at(base_fd, (off_t)old_kv_off, dst,
                      (off_t)h.kv_off, (size_t)old_kv_size) != 0) goto fail;
    if (old_st_size > 0 &&
        copy_plain_at(base_fd, (off_t)old_st_off, dst,
                      (off_t)h.st_off, (size_t)old_st_size) != 0) goto fail;
    if (old_sal_size > 0 &&
        copy_plain_at(base_fd, (off_t)old_sal_off, dst,
                      (off_t)h.sal_off, (size_t)old_sal_size) != 0) goto fail;
    if (ftruncate(dst, (off_t)total) != 0) goto fail;
    if (pwrite(dst, &h, sizeof(h), (off_t)(total - sizeof(h))) != (ssize_t)sizeof(h))
        goto fail;
    fsync(dst);
    close(dst);
    close(mfd);
