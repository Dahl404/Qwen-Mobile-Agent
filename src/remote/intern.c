 ...
 * Self-contained: no engine dependencies. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdarg.h>
...
static int junk_path(const char *rel) {
    ...
    /* derived emotion vectors (gop): the .txt probe files embed (they are
       the source of truth); the .bin vectors are model-specific artifacts
       rebuilt with --build-emotions — keep the self-hosting blob small */
    if (l > 4 && strcmp(base + l - 4, ".bin") == 0) return 1;
    return 0;
}
...
void intern_build_cmd(char *buf, size_t cap, const char *src, const char *out_bin) {
    /* MUST match the remote agent's Makefile (LFM engine + remote shell) */
    static const char *files[] = {
        "lfm_gguf.c", "lfm_tokenizer.c", "lfm_nn.c", "lfm_sampler.c",
        "remote_main.c", "remote_tools.c", "selfctx.c", "intern.c",
        "platform_id.c",
    };
    /* tune to the compiling machine (same rule as the Makefile): ARM -mcpu,
       x86 -march. The compiler is detected at runtime (clang -> cc -> gcc)
       so the self-rebuild works on any platform. */
    const char *tune;
    ...
    snprintf(buf, cap,
        ...
        "else echo 'no C compiler found (need clang/cc/gcc)'; exit 127; fi; "
        "$CC -O3 -std=c99 -Wall -Wextra -Wno-unused-parameter "
        "%s -ffast-math -fomit-frame-pointer -pthread -o %s",
        tune, out_bin);
    size_t o = strlen(buf);
    o += (size_t)snprintf(buf + o, cap - o, " -I%s -I%s/lfm -I%s/remote", src, src, src);
    for (size_t i = 0; i < sizeof(files) / sizeof(files[0]); i++) {
        const char *sub = strncmp(files[i], "lfm_", 4) == 0 ? "lfm" : "remote";
        int w = snprintf(buf + o, cap - o, " %s/%s/%s", src, sub, files[i]);
...
int sys_run_capture(const char *cmd, int timeout_s, char *out, size_t cap) {
    ...
    if (pid == 0) {
        ...
        dup2(pipefd[1], 2);
        close(pipefd[1]);
        execlp("bash", "bash", "-c", cmd, (char *)NULL);   /* PATH lookup */
        execlp("sh", "sh", "-c", cmd, (char *)NULL);       /* minimal fallback */
        _exit(127);
    }
    close(pipefd[1]);
    size_t o = 0;
...
/* Recursively delete a directory tree (used to prune stale mounts). */
int intern_rmtree(const char *path) {
    ...
    while ((e = readdir(d)) != NULL) {
        ...
        struct stat st;
        if (lstat(p, &st) == 0 && S_ISDIR(st.st_mode)) intern_rmtree(p);
        else unlink(p);
    }
    closedir(d);
    rmdir(path);
