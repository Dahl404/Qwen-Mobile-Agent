 ...
 *   lfm_samp_candidates() — repeat penalty + temp + top-k  -> candidate list
 *   lfm_samp_pick()       — grammar-masked candidates + top-p + sample
...
void lfm_samp_init(lfm_samp_t *s, uint64_t seed, float temp, int top_k, float top_p,
...
int lfm_samp_pick(lfm_samp_t *s, const int *ids, float *lgs, int n, float top_p) {
    ...
    /* all candidates masked (grammar left nothing legal in the window):
       return -1 so the caller can find a legal token in the full vocab —
       NEVER fall back to the masked top token (that would emit an illegal
       char and corrupt an open tool call). */
    if (first_live < 0) return -1;
